...
class Dominance_Eq(DataFlowEq):
    ...
    def eval_aux(self, env):
        ...
        if(len(self.inst.preds) > 0):
            sol = UniversalSet()
            for p in self.inst.preds:
                sol = sol & env[str(p.ID)]

            if isinstance(sol, UniversalSet):
                return sol
            else:
                return sol | set([self.inst.ID])
        else:
            return set([self.inst.ID])
