...
class Env:
    ...
    def get_from_list(self, vars):
        ...
        if val is not None:
            ...
            raise LookupError(f"Absent keys {vars}")
...
class InstTypeErr(Exception):
    def __init__(s, inst, expected, found):
        s.inst = inst
        s.expected = expected
        s.found = found
        message = (
...
class Phi(Inst):
    ...
    def type_eval(self, type_env):
        ...
        for var in self.args:
            try:
                varT = type_env.get(var)
            except LookupError:
                varT = None

            if currT is None and varT is not None:
                currT = varT
...
class PhiBlock(Inst):
    ...
    def eval(self, env, PC):
        ...
        # We read all the values without updating the enviroment
...
class Add(BinOp):
    ...
    def type_eval(self, type_env:TypeEnv):
        """
        ...
        type_env.set(self.dst, LangType.NUM) 
        # raise NotImplementedError
...
class Lth(BinOp):
    ...
    def type_eval(self, type_env):
        ...
        type_env.set(self.dst, LangType.BOOL) 
        # raise NotImplementedError
...
def type_check(inst, tp_env, phi_queue = []):
    """
    ...
    if inst:
        ...
        type_check(inst.get_next(), tp_env, phi_queue)
    else:
